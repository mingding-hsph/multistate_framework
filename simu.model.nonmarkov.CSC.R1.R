...
for (ps in 510:paper_simu){
  ...
  pfit01 <- predict(cfit0, newdata = new_data0, cause = 1, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit02 <- predict(cfit0, newdata = new_data0, cause = 2, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit03 <- predict(cfit0, newdata = new_data0, cause = 3, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit04 <- predict(cfit0, newdata = new_data0, cause = 4, times =break_interval, se = TRUE, keep.newdata = FALSE)
  
  predict0<-data.frame(new_data0$interval, 
                       pfit01$absRisk, pfit01$absRisk.se, 
                       pfit02$absRisk, pfit02$absRisk.se, 
                       pfit03$absRisk, pfit03$absRisk.se, 
                       pfit04$absRisk, pfit04$absRisk.se)
  ...
  pfit12 <- predict(cfit1, newdata = new_data0, cause = 2, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit13 <- predict(cfit1, newdata = new_data0, cause = 3, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit14 <- predict(cfit1, newdata = new_data0, cause = 4, times =break_interval, se = TRUE, keep.newdata = FALSE)
  
  predict1<-data.frame(new_data0$interval, 
                       pfit12$absRisk, pfit12$absRisk.se, 
                       pfit13$absRisk, pfit13$absRisk.se, 
                       pfit14$absRisk, pfit14$absRisk.se)
  ...
  cfit2 <- CSC(formula = list(Hist(tte_cmr,S2_mylong) ~ 
                                as.factor(sub2)+ rcs(interval,3) + as.factor(sub2)*rcs(interval,3))
               , data = data2_long)
  
  new_data2_0<-NA
  new_data2_0=data.frame(c(1:analysis_interval))
  colnames(new_data2_0)=c("interval")
  new_data2_0$sub2='well-chd'
  ...
  pfit23_0 <- predict(cfit2, newdata = new_data2_0, cause = 3, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit24_0 <- predict(cfit2, newdata = new_data2_0, cause = 4, times =break_interval, se = TRUE, keep.newdata = FALSE)
  
  predict2_0<-data.frame(new_data2_0$interval, new_data2_0$sub2, 
                         pfit23_0$absRisk, pfit23_0$absRisk.se, 
                         pfit24_0$absRisk, pfit24_0$absRisk.se)
  
  colnames(predict2_0)=c("interval","sub2", "r23", "se_r23","r24", "se_r24")
  
  
  pfit23_1 <- predict(cfit2, newdata = new_data2_1, cause = 3, times =break_interval, se = TRUE, keep.newdata = FALSE)
  pfit24_1 <- predict(cfit2, newdata = new_data2_1, cause = 4, times =break_interval, se = TRUE, keep.newdata = FALSE)
  
  predict2_1<-data.frame(new_data2_1$interval, new_data2_1$sub2, 
                         pfit23_1$absRisk, pfit23_1$absRisk.se, 
                         pfit24_1$absRisk, pfit24_1$absRisk.se)
  
  colnames(predict2_1)=c("interval","sub2", "r23", "se_r23","r24", "se_r24")
