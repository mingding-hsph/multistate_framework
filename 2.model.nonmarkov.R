...
setwd("./0.5k.nonmarkov.data/R2.data")
break_interval=0.1
...
if (nrow(as.matrix(table(data2_long$sub2)))==2){
  ...
  pfit23_0 <- predict(cfit2, newdata = new_data2_0, cause = 3, times =break_interval, se = F, keep.newdata = FALSE)
  pfit24_0 <- predict(cfit2, newdata = new_data2_0, cause = 4, times =break_interval, se = F, keep.newdata = FALSE)
  
  pfit23_1 <- predict(cfit2, newdata = new_data2_1, cause = 3, times =break_interval, se = F, keep.newdata = FALSE)
  pfit24_1 <- predict(cfit2, newdata = new_data2_1, cause = 4, times =break_interval, se = F, keep.newdata = FALSE)
  
} else {
  pfit23_0$absRisk=matrix(NA, nrow=nrow(new_data2_0), ncol=1)
  pfit24_0$absRisk=matrix(NA, nrow=nrow(new_data2_0), ncol=1)

  pfit23_1$absRisk=matrix(NA, nrow=nrow(new_data2_1), ncol=1)
  pfit24_1$absRisk=matrix(NA, nrow=nrow(new_data2_1), ncol=1)
  ...
  colnames(predict2_0)=c("interval","sub2", "r23","r24")
  ...
  colnames(predict2_1)=c("interval","sub2", "r23", "r24")
...
if (nrow(as.matrix(table(data3_long$sub3)))==4){
