...
data0_all = stringr::str_sort(list.files(pattern="data0_.*txt",  full.names=TRUE), num = TRUE)
data1_all = stringr::str_sort(list.files(pattern="data1_.*txt",  full.names=TRUE), num = TRUE)
data2_all = stringr::str_sort(list.files(pattern="data2_.*txt",  full.names=TRUE), num = TRUE)
data3_all = stringr::str_sort(list.files(pattern="data3_.*txt",  full.names=TRUE), num = TRUE)
...
for (ps in 1:paper_simu){
  ...
  data0<-read.table(data0_all[ps])
  data1<-read.table(data1_all[ps])
  data2<-read.table(data2_all[ps])
  data3<-read.table(data3_all[ps])
...
##also tried pspline(interval,df=4), however, pspline cannot output SE using predictCox
  
cfit01 <- coxph(Surv(tte_cmr,S0_y_1) ~ rcs(interval,4), data = data0_long, x = TRUE)
cfit02 <- coxph(Surv(tte_cmr,S0_y_2) ~ rcs(interval,4), data = data0_long, x = TRUE)
cfit03 <- coxph(Surv(tte_cmr,S0_y_3) ~ rcs(interval,4), data = data0_long, x = TRUE)
cfit04 <- coxph(Surv(tte_cmr,S0_y_4) ~ rcs(interval,4), data = data0_long, x = TRUE)

new_data0=data.frame(c(1:analysis_interval))
colnames(new_data0)=c("interval")

pfit01 <- predictCox(cfit01, newdata=new_data0, times=break_interval, se=T)
pfit02 <- predictCox(cfit02, newdata=new_data0, times=break_interval, se=T)
pfit03 <- predictCox(cfit03, newdata=new_data0, times=break_interval, se=T)
pfit04 <- predictCox(cfit04, newdata=new_data0, times=break_interval, se=T)

predict0<-data.frame(new_data0$interval, 
                     pfit01$survival, pfit01$survival.lower, pfit01$survival.upper,
                     pfit02$survival, pfit02$survival.lower, pfit02$survival.upper,
                     pfit03$survival, pfit03$survival.lower, pfit03$survival.upper,
                     pfit04$survival, pfit04$survival.lower, pfit04$survival.upper)
  
colnames(predict0)=c("interval",
                     "r01_s", "r01_s_ll","r01_s_ul", 
                     "r02_s", "r02_s_ll","r02_s_ul", 
                     "r03_s", "r03_s_ll","r03_s_ul", 
                     "r04_s", "r04_s_ll","r04_s_ul") 
...
predict0<-predict0 %>%
  mutate(
    ...
    r01_ul=(1-r01_s_ll), 
    r01_ll=(1-r01_s_ul), 
    se_r01=-(r01_ll-r01_ul)/3.92,
    
    
    r02=(1-r02_s),
    r02_ul=(1-r02_s_ll), 
    r02_ll=(1-r02_s_ul), 
    se_r02=-(r02_ll-r02_ul)/3.92,
    
    
    r03=(1-r03_s),
    r03_ul=(1-r03_s_ll), 
    r03_ll=(1-r03_s_ul), 
    se_r03=-(r03_ll-r03_ul)/3.92,
    
    
    r04=(1-r04_s),
    r04_ul=(1-r04_s_ll), 
    r04_ll=(1-r04_s_ul),
    se_r04=-(r04_ll-r04_ul)/3.92
  ) 

##data1

cfit12 <- coxph(Surv(tte_cmr,S1_y_2) ~ rcs(interval,4), data = data1_long, x = TRUE)
cfit13 <- coxph(Surv(tte_cmr,S1_y_3) ~ rcs(interval,4), data = data1_long, x = TRUE)
cfit14 <- coxph(Surv(tte_cmr,S1_y_4) ~ rcs(interval,4), data = data1_long, x = TRUE)

new_data1=data.frame(c(1:analysis_interval))
colnames(new_data1)=c("interval")

pfit12 <- predictCox(cfit12, newdata=new_data1, times=break_interval, se=T)
pfit13 <- predictCox(cfit13, newdata=new_data1, times=break_interval, se=T)
pfit14 <- predictCox(cfit14, newdata=new_data1, times=break_interval, se=T)

predict1<-data.frame(new_data1$interval, 
                     pfit12$survival, pfit12$survival.lower, pfit12$survival.upper,
                     pfit13$survival, pfit13$survival.lower, pfit13$survival.upper,
                     pfit14$survival, pfit14$survival.lower, pfit14$survival.upper)

colnames(predict1)=c("interval",
                     "r12_s", "r12_s_ll","r12_s_ul", 
                     "r13_s", "r13_s_ll","r13_s_ul", 
                     "r14_s", "r14_s_ll","r14_s_ul") 
...
predict1<-predict1 %>%
  mutate(
    ...
    r12_ul=(1-r12_s_ll), 
    r12_ll=(1-r12_s_ul), 
    se_r12=-(r12_ll-r12_ul)/3.92,
    
    
    r13=(1-r13_s),
    r13_ul=(1-r13_s_ll), 
    r13_ll=(1-r13_s_ul), 
    se_r13=-(r13_ll-r13_ul)/3.92,
    
    
    r14=(1-r14_s),
    r14_ul=(1-r14_s_ll), 
    r14_ll=(1-r14_s_ul),
    se_r14=-(r14_ll-r14_ul)/3.92
  ) 

###data 2

cfit23 <- coxph(Surv(tte_cmr,S2_y_3) ~ 
                  as.factor(sub2)+ rcs(interval,3)
                + as.factor(sub2)*rcs(interval,3)
                , data = data2_long, x = TRUE)

summary(cfit23)

cfit24 <- coxph(Surv(tte_cmr,S2_y_4) ~ 
                  as.factor(sub2)+ rcs(interval,3)
                + as.factor(sub2)*rcs(interval,3)
                , data = data2_long, x = TRUE)

new_data2_1<-NA
new_data2_1=data.frame(c(1:analysis_interval))
colnames(new_data2_1)=c("interval")
new_data2_1$sub2="htn-chd"

new_data2_2<-NA
new_data2_2=data.frame(c(1:analysis_interval))
colnames(new_data2_2)=c("interval")
new_data2_2$sub2="well-chd"

new_data2=rbind(new_data2_1,new_data2_2)

pfit23 <- predictCox(cfit23, newdata=new_data2, times=break_interval, se=T)
pfit24 <- predictCox(cfit24, newdata=new_data2, times=break_interval, se=T)

predict2<-data.frame(new_data2$interval, new_data2$sub2,
                     pfit23$survival, pfit23$survival.lower, pfit23$survival.upper,
                     pfit24$survival, pfit24$survival.lower, pfit24$survival.upper)

colnames(predict2)=c("interval","sub2",
                     "r23_s", "r23_s_ll","r23_s_ul", 
                     "r24_s", "r24_s_ll","r24_s_ul") 
...
predict2<-predict2 %>%
  mutate(
    ...
    r23_ul=(1-r23_s_ll), 
    r23_ll=(1-r23_s_ul), 
    se_r23=-(r23_ll-r23_ul)/3.92,
    
    r24=(1-r24_s),
    r24_ul=(1-r24_s_ll), 
    r24_ll=(1-r24_s_ul),
    se_r24=-(r24_ll-r24_ul)/3.92
  ) 
...
##data3
  ...
  cfit34 <- coxph(Surv(tte_cmr,S3_y_4) ~ 
                    as.factor(sub3)+ rcs(interval,4)
                  +as.factor(sub3)*rcs(interval,4)
                  , 
                  data = data3_long, x = TRUE)
  
  new_data3_1<-NA
  new_data3_1=data.frame(c(1:analysis_interval))
  colnames(new_data3_1)=c("interval")
  new_data3_1$sub3="chd-chf"
  
  new_data3_2<-NA
  new_data3_2=data.frame(c(1:analysis_interval))
  colnames(new_data3_2)=c("interval")
  new_data3_2$sub3="chf"
  
  new_data3_3<-NA
  new_data3_3=data.frame(c(1:analysis_interval))
  colnames(new_data3_3)=c("interval")
  new_data3_3$sub3="htn-chd-chf"
  
  new_data3_4<-NA
  new_data3_4=data.frame(c(1:analysis_interval))
  colnames(new_data3_4)=c("interval")
  new_data3_4$sub3="htn-chf"
  
  new_data3=rbind(new_data3_1,new_data3_2, new_data3_3, new_data3_4)
  
  pfit34 <- predictCox(cfit34, newdata=new_data3, times=break_interval, se = TRUE)
  
  predict3<-data.frame(new_data3$interval, new_data3$sub3,
                       pfit34$survival, pfit34$survival.lower, pfit34$survival.upper)
  
  colnames(predict3)=c("interval", "sub3",
                       "r34_s", "r34_s_ll","r34_s_ul") 
...
for (simu in 1:var_loop){
    ...
    for (t in 1:analysis_interval) {
      ...
      TR_var[[t+(simu-1)*analysis_interval]][1,2]=rnorm(1,predict0$r01[t],(predict0$se_r01[t]))
      TR_var[[t+(simu-1)*analysis_interval]][1,3]=rnorm(1,predict0$r02[t],(predict0$se_r02[t]))
      TR_var[[t+(simu-1)*analysis_interval]][1,5]=rnorm(1,predict0$r03[t],(predict0$se_r03[t]))
      TR_var[[t+(simu-1)*analysis_interval]][1,9]=rnorm(1,predict0$r04[t],(predict0$se_r04[t]))
      
      
      TR_var[[t+(simu-1)*analysis_interval]][2,4]=rnorm(1,predict1$r12[t],(predict1$se_r12[t]))
      TR_var[[t+(simu-1)*analysis_interval]][2,6]=rnorm(1,predict1$r13[t],(predict1$se_r13[t]))
      TR_var[[t+(simu-1)*analysis_interval]][2,9]=rnorm(1,predict1$r14[t],(predict1$se_r14[t]))
      
      TR_var[[t+(simu-1)*analysis_interval]][3,7]=rnorm(1,predict2_0$r23[t],(predict2_0$se_r23[t]))
      TR_var[[t+(simu-1)*analysis_interval]][3,9]=rnorm(1,predict2_0$r24[t],(predict2_0$se_r24[t]))
      
      TR_var[[t+(simu-1)*analysis_interval]][4,8]=rnorm(1,predict2_1$r23[t],(predict2_1$se_r23[t]))
      TR_var[[t+(simu-1)*analysis_interval]][4,9]=rnorm(1,predict2_1$r24[t],(predict2_1$se_r24[t]))
      
      TR_var[[t+(simu-1)*analysis_interval]][5,9]=rnorm(1,predict3_0$r34[t],(predict3_0$se_r34[t]))
      
      TR_var[[t+(simu-1)*analysis_interval]][6,9]=rnorm(1,predict3_01$r34[t],(predict3_01$se_r34[t]))
      
      TR_var[[t+(simu-1)*analysis_interval]][7,9]=rnorm(1,predict3_02$r34[t],(predict3_02$se_r34[t]))
      
      TR_var[[t+(simu-1)*analysis_interval]][8,9]=rnorm(1,predict3_012$r34[t],(predict3_012$se_r34[t]))
  ...
  #organize the results from 1000 bootstrap
  ...
  for (h in 1:analysis_interval) {
    ...
    TR_11_final[h,]=quantile(TR_var_11[,h], probs = c(0.025, 0.5, 0.975))
    TR_12_final[h,]=quantile(TR_var_12[,h], probs = c(0.025, 0.5, 0.975))
    TR_13_final[h,]=quantile(TR_var_13[,h], probs = c(0.025, 0.5, 0.975))
    TR_14_final[h,]=quantile(TR_var_14[,h], probs = c(0.025, 0.5, 0.975))
    TR_15_final[h,]=quantile(TR_var_15[,h], probs = c(0.025, 0.5, 0.975))
    TR_16_final[h,]=quantile(TR_var_16[,h], probs = c(0.025, 0.5, 0.975))
    TR_17_final[h,]=quantile(TR_var_17[,h], probs = c(0.025, 0.5, 0.975))
    TR_18_final[h,]=quantile(TR_var_18[,h], probs = c(0.025, 0.5, 0.975))
    TR_19_final[h,]=quantile(TR_var_19[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_22_final[h,]=quantile(TR_var_22[,h], probs = c(0.025, 0.5, 0.975))
    TR_23_final[h,]=quantile(TR_var_23[,h], probs = c(0.025, 0.5, 0.975))
    TR_24_final[h,]=quantile(TR_var_24[,h], probs = c(0.025, 0.5, 0.975))
    TR_25_final[h,]=quantile(TR_var_25[,h], probs = c(0.025, 0.5, 0.975))
    TR_26_final[h,]=quantile(TR_var_26[,h], probs = c(0.025, 0.5, 0.975))
    TR_27_final[h,]=quantile(TR_var_27[,h], probs = c(0.025, 0.5, 0.975))
    TR_28_final[h,]=quantile(TR_var_28[,h], probs = c(0.025, 0.5, 0.975))
    TR_29_final[h,]=quantile(TR_var_29[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_33_final[h,]=quantile(TR_var_33[,h], probs = c(0.025, 0.5, 0.975))
    TR_34_final[h,]=quantile(TR_var_34[,h], probs = c(0.025, 0.5, 0.975))
    TR_35_final[h,]=quantile(TR_var_35[,h], probs = c(0.025, 0.5, 0.975))
    TR_36_final[h,]=quantile(TR_var_36[,h], probs = c(0.025, 0.5, 0.975))
    TR_37_final[h,]=quantile(TR_var_37[,h], probs = c(0.025, 0.5, 0.975))
    TR_38_final[h,]=quantile(TR_var_38[,h], probs = c(0.025, 0.5, 0.975))
    TR_39_final[h,]=quantile(TR_var_39[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_44_final[h,]=quantile(TR_var_44[,h], probs = c(0.025, 0.5, 0.975))
    TR_45_final[h,]=quantile(TR_var_45[,h], probs = c(0.025, 0.5, 0.975))
    TR_46_final[h,]=quantile(TR_var_46[,h], probs = c(0.025, 0.5, 0.975))
    TR_47_final[h,]=quantile(TR_var_47[,h], probs = c(0.025, 0.5, 0.975))
    TR_48_final[h,]=quantile(TR_var_48[,h], probs = c(0.025, 0.5, 0.975))
    TR_49_final[h,]=quantile(TR_var_49[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_55_final[h,]=quantile(TR_var_55[,h], probs = c(0.025, 0.5, 0.975))
    TR_56_final[h,]=quantile(TR_var_56[,h], probs = c(0.025, 0.5, 0.975))
    TR_57_final[h,]=quantile(TR_var_57[,h], probs = c(0.025, 0.5, 0.975))
    TR_58_final[h,]=quantile(TR_var_58[,h], probs = c(0.025, 0.5, 0.975))
    TR_59_final[h,]=quantile(TR_var_59[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_66_final[h,]=quantile(TR_var_66[,h], probs = c(0.025, 0.5, 0.975))
    TR_67_final[h,]=quantile(TR_var_67[,h], probs = c(0.025, 0.5, 0.975))
    TR_68_final[h,]=quantile(TR_var_68[,h], probs = c(0.025, 0.5, 0.975))
    TR_69_final[h,]=quantile(TR_var_69[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_77_final[h,]=quantile(TR_var_77[,h], probs = c(0.025, 0.5, 0.975))
    TR_78_final[h,]=quantile(TR_var_78[,h], probs = c(0.025, 0.5, 0.975))
    TR_79_final[h,]=quantile(TR_var_79[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_88_final[h,]=quantile(TR_var_88[,h], probs = c(0.025, 0.5, 0.975))
    TR_89_final[h,]=quantile(TR_var_89[,h], probs = c(0.025, 0.5, 0.975))
    
    TR_99_final[h,]=quantile(TR_var_99[,h], probs = c(0.025, 0.5, 0.975))
    
    
    
    TP_11_final[h,]=quantile(TP_var_11[,h], probs = c(0.025, 0.5, 0.975))
    TP_12_final[h,]=quantile(TP_var_12[,h], probs = c(0.025, 0.5, 0.975))
    TP_13_final[h,]=quantile(TP_var_13[,h], probs = c(0.025, 0.5, 0.975))
    TP_14_final[h,]=quantile(TP_var_14[,h], probs = c(0.025, 0.5, 0.975))
    TP_15_final[h,]=quantile(TP_var_15[,h], probs = c(0.025, 0.5, 0.975))
    TP_16_final[h,]=quantile(TP_var_16[,h], probs = c(0.025, 0.5, 0.975))
    TP_17_final[h,]=quantile(TP_var_17[,h], probs = c(0.025, 0.5, 0.975))
    TP_18_final[h,]=quantile(TP_var_18[,h], probs = c(0.025, 0.5, 0.975))
    TP_19_final[h,]=quantile(TP_var_19[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_22_final[h,]=quantile(TP_var_22[,h], probs = c(0.025, 0.5, 0.975))
    TP_23_final[h,]=quantile(TP_var_23[,h], probs = c(0.025, 0.5, 0.975))
    TP_24_final[h,]=quantile(TP_var_24[,h], probs = c(0.025, 0.5, 0.975))
    TP_25_final[h,]=quantile(TP_var_25[,h], probs = c(0.025, 0.5, 0.975))
    TP_26_final[h,]=quantile(TP_var_26[,h], probs = c(0.025, 0.5, 0.975))
    TP_27_final[h,]=quantile(TP_var_27[,h], probs = c(0.025, 0.5, 0.975))
    TP_28_final[h,]=quantile(TP_var_28[,h], probs = c(0.025, 0.5, 0.975))
    TP_29_final[h,]=quantile(TP_var_29[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_33_final[h,]=quantile(TP_var_33[,h], probs = c(0.025, 0.5, 0.975))
    TP_34_final[h,]=quantile(TP_var_34[,h], probs = c(0.025, 0.5, 0.975))
    TP_35_final[h,]=quantile(TP_var_35[,h], probs = c(0.025, 0.5, 0.975))
    TP_36_final[h,]=quantile(TP_var_36[,h], probs = c(0.025, 0.5, 0.975))
    TP_37_final[h,]=quantile(TP_var_37[,h], probs = c(0.025, 0.5, 0.975))
    TP_38_final[h,]=quantile(TP_var_38[,h], probs = c(0.025, 0.5, 0.975))
    TP_39_final[h,]=quantile(TP_var_39[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_44_final[h,]=quantile(TP_var_44[,h], probs = c(0.025, 0.5, 0.975))
    TP_45_final[h,]=quantile(TP_var_45[,h], probs = c(0.025, 0.5, 0.975))
    TP_46_final[h,]=quantile(TP_var_46[,h], probs = c(0.025, 0.5, 0.975))
    TP_47_final[h,]=quantile(TP_var_47[,h], probs = c(0.025, 0.5, 0.975))
    TP_48_final[h,]=quantile(TP_var_48[,h], probs = c(0.025, 0.5, 0.975))
    TP_49_final[h,]=quantile(TP_var_49[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_55_final[h,]=quantile(TP_var_55[,h], probs = c(0.025, 0.5, 0.975))
    TP_56_final[h,]=quantile(TP_var_56[,h], probs = c(0.025, 0.5, 0.975))
    TP_57_final[h,]=quantile(TP_var_57[,h], probs = c(0.025, 0.5, 0.975))
    TP_58_final[h,]=quantile(TP_var_58[,h], probs = c(0.025, 0.5, 0.975))
    TP_59_final[h,]=quantile(TP_var_59[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_66_final[h,]=quantile(TP_var_66[,h], probs = c(0.025, 0.5, 0.975))
    TP_67_final[h,]=quantile(TP_var_67[,h], probs = c(0.025, 0.5, 0.975))
    TP_68_final[h,]=quantile(TP_var_68[,h], probs = c(0.025, 0.5, 0.975))
    TP_69_final[h,]=quantile(TP_var_69[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_77_final[h,]=quantile(TP_var_77[,h], probs = c(0.025, 0.5, 0.975))
    TP_78_final[h,]=quantile(TP_var_78[,h], probs = c(0.025, 0.5, 0.975))
    TP_79_final[h,]=quantile(TP_var_79[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_88_final[h,]=quantile(TP_var_88[,h], probs = c(0.025, 0.5, 0.975))
    TP_89_final[h,]=quantile(TP_var_89[,h], probs = c(0.025, 0.5, 0.975))
    
    TP_99_final[h,]=quantile(TP_var_99[,h], probs = c(0.025, 0.5, 0.975))
    
    
    
    SOP_1_final[h,]=quantile(SOP_var_1[,h], probs = c(0.025, 0.5, 0.975))
    SOP_2_final[h,]=quantile(SOP_var_2[,h], probs = c(0.025, 0.5, 0.975))
    SOP_3_final[h,]=quantile(SOP_var_3[,h], probs = c(0.025, 0.5, 0.975))
    SOP_4_final[h,]=quantile(SOP_var_4[,h], probs = c(0.025, 0.5, 0.975))
    SOP_5_final[h,]=quantile(SOP_var_5[,h], probs = c(0.025, 0.5, 0.975))
    SOP_6_final[h,]=quantile(SOP_var_6[,h], probs = c(0.025, 0.5, 0.975))
    SOP_7_final[h,]=quantile(SOP_var_7[,h], probs = c(0.025, 0.5, 0.975))
    SOP_8_final[h,]=quantile(SOP_var_8[,h], probs = c(0.025, 0.5, 0.975))
    SOP_9_final[h,]=quantile(SOP_var_9[,h], probs = c(0.025, 0.5, 0.975))
  ...
  write.table(TR_t1_table, file.path("./non.markov.model/", paste0("TR_t1_",ps,".txt")))
  write.table(TR_t2_table, file.path("./non.markov.model/", paste0("TR_t2_",ps,".txt")))
  write.table(TR_t3_table, file.path("./non.markov.model/", paste0("TR_t3_",ps,".txt")))
  write.table(TR_t4_table, file.path("./non.markov.model/", paste0("TR_t4_",ps,".txt")))
  write.table(TR_t5_table, file.path("./non.markov.model/", paste0("TR_t5_",ps,".txt")))
  ...
  write.table(TP_t1_table, file.path("./non.markov.model/", paste0("TP_t1_",ps,".txt")))
  write.table(TP_t2_table, file.path("./non.markov.model/", paste0("TP_t2_",ps,".txt")))
  write.table(TP_t3_table, file.path("./non.markov.model/", paste0("TP_t3_",ps,".txt")))
  write.table(TP_t4_table, file.path("./non.markov.model/", paste0("TP_t4_",ps,".txt")))
  write.table(TP_t5_table, file.path("./non.markov.model/", paste0("TP_t5_",ps,".txt")))
